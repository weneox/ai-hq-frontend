...
function fieldClassName() {
  return "w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-slate-300 focus:ring-4 focus:ring-slate-100";
}

function textAreaClassName(minHeightClass = "min-h-[120px]") {
  return `${minHeightClass} w-full rounded-2xl border border-slate-200 bg-white px-4 py-3.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-slate-300 focus:ring-4 focus:ring-slate-100`;
}

function labelClassName() {
  return "mb-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500";
}
...
export default function SetupStudioRefineModal({
  ...
  const manual = obj(manualSections);

  const servicesText = s(manual.servicesText);
  const faqsText = s(manual.faqsText);
  const policiesText = s(manual.policiesText);

  const serviceCount = countLines(servicesText);
  const faqCount = countLines(faqsText);
  const policyCount = countLines(policiesText);
  ...
  return (
    <motion.div
      ...
      className="pointer-events-auto relative mx-auto flex w-full max-w-[1180px] flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_30px_100px_rgba(15,23,42,0.18)]"
      style={{ maxHeight: "calc(100vh - 32px)" }}
    >
      <div className="flex items-start justify-between gap-4 border-b border-slate-200 bg-white px-5 py-5 sm:px-6">
        <div className="min-w-0">
          <Pill icon={Brain}>Refine draft</Pill>

          <h2 className="mt-4 text-2xl font-semibold tracking-[-0.04em] text-slate-950 sm:text-[30px]">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        <div className="grid gap-6 px-5 py-5 sm:px-6 xl:grid-cols-[minmax(0,1.08fr)_minmax(340px,0.92fr)]">
          <form
            id={FORM_ID}
          ...
          <div className="min-w-0 space-y-6">
            <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,.05)]">
              <Pill icon={BadgeCheck}>Extracted snapshot</Pill>
