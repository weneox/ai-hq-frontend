...
function fieldClassName({ multiline = false, needsAttention = false } = {}) {
  return cx(
    multiline ? "min-h-[124px] resize-none py-3.5" : "h-11",
    "w-full rounded-[18px] border px-4 text-[14px] text-slate-950 outline-none transition placeholder:text-slate-400",
    needsAttention
      ? "border-amber-200 bg-amber-50/70 focus:border-amber-300"
      : "border-slate-200 bg-white/92 focus:border-slate-300"
  );
}

function Field({ label, value, onChange, placeholder, multiline = false, needsAttention = false }) {
  ...
  return (
    <div className="min-w-0">
      <div className="mb-2 flex items-center justify-between gap-3">
        ...
        {needsAttention ? (
          <TinyChip tone="warn">Needs review</TinyChip>
        ) : null}
      </div>
...
export default function SetupStudioRefineModal({
  ...
  return (
    <motion.div
      ...
      className="relative my-2 flex max-h-[calc(100vh-1rem)] w-full max-w-[1120px] flex-col overflow-hidden rounded-[34px] border border-white/72 bg-[linear-gradient(180deg,rgba(255,255,255,.92)_0%,rgba(246,247,249,.86)_100%)] shadow-[0_34px_90px_-40px_rgba(15,23,42,.42)] sm:my-4 sm:max-h-[calc(100vh-2rem)]"
    >
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(780px_circle_at_top_left,rgba(255,255,255,.92),transparent_42%),radial-gradient(620px_circle_at_bottom_right,rgba(226,232,240,.28),transparent_34%)]" />
      ...
      <div className="relative z-10 min-h-0 flex-1 overflow-y-auto">
        <form
          ...
          className="mx-auto flex w-full max-w-[980px] min-w-0 flex-col gap-5 px-5 py-5 sm:px-6"
        >
          {issues.length ? (
          ...
          <StagePanel className="space-y-5">
            <SectionHeading
              ...
              body="Edit the draft values you want to save."
            />
          ...
          <StagePanel className="space-y-5">
            <SectionHeading
              ...
              body="Keep each section short and easy to review."
            />
          ...
          {(rows.length || sources.length) ? (
            <StagePanel tone="subtle" className="space-y-5">
              <SectionHeading
                label="Observed evidence"
                title="Secondary reference"
                body="Observed values are kept separate from the editable draft."
              />
            ...
            <StagePanel tone="subtle">
              <EmptyBlock
                title="No observed evidence"
                body="This draft can still be completed manually."
              />
            </StagePanel>
          )}
        </form>
